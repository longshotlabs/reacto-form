  ...
  renderFormFields() {
    ...
    const propsFunc = (element) => {
      ...
      if (element.type.isFormField) {
        ...
        newProps[propNames.onChange] = this.getFieldOnChangeHandler(name, element.props[propNames.onChange]);
        newProps[propNames.onChanging] = this.getFieldOnChangingHandler(name, element.props[propNames.onChanging]);
        ...
        if (element.props[propNames.value] === undefined) {
          ...
          let inputValue = get(value, name, null);
          if (inputValue === null && nullValue !== undefined) inputValue = nullValue;
          newProps[propNames.value] = inputValue;
        }

        if (element.props[propNames.errors] === undefined) {
          newProps[propNames.errors] = filterErrorsForNames(errors, [name], false);
