 ...
 */
export default function recursivelyCloneElements(elements, getProps, shouldStopRecursing) {
  const newElements = React.Children.map(elements, (element) => {
    ...
    if (typeof shouldStopRecursing !== 'function') shouldStopRecursing = () => false;

    const children = shouldStopRecursing(element) ? element.props.children : recursivelyCloneElements(element.props.children, getProps);
