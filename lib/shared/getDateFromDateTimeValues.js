...
export default function getDateFromDateTimeValues(obj, moment, timezone) {
  ...
  if (typeof dayValue !== "string" || !dayValue.match(oneOrTwoDigits))
    return null;
  if (
    typeof monthValue !== "string" ||
    !monthValue.match(oneOrTwoDigits) ||
    Number(monthValue) > 12 ||
    Number(monthValue) < 1
  )
    return null;
  if (typeof timeValue !== "string" || !timeValue.match(timeFormat))
    return null;
  if (typeof yearValue !== "string" || !yearValue.match(upToFourDigits))
    return null;
